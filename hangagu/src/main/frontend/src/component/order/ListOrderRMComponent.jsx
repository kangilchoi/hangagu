...
function ListOrderComponent({location}) {
    ...
    const [odStatus, setOdStatus] = useState(["CANCEL", "RETURN", "EXCHANGE", "RETURN"]);
    ...
    useEffect(() => {
        ...
        const fetchOrder = async() => {
            try {
                ...
                OrderService.getOrders(fromDt, toDt, page, memKey, odStatus).then(res => {
    ...
    const setOrderStatus = (content) => {
        for(var i = 0; i < content.length; i++) {
            ...
            if(content[i].odStatus === "BEFORE_DEPOSIT") {
                ...
                content[i].btnStatus = <a href="#none" className="btnUpdate"><label onClick={() => modOrder(odKey, "CANCEL", cartKey)}>취소 &gt;</label><label onClick={() => modOrder(odKey, "EXCHANGE", cartKey)}>교환 &gt;</label><label onClick={() => modOrder(odKey, "RETURN", cartKey)}>반품 &gt;</label></a>;
    ...
    const searchList = async(page) => {
        try {
            ...
            OrderService.getOrders(fromDt, toDt, page, document.querySelector("#memKey").value, odStatus).then(res => {
    ...
    const toList = () => {
        window.location.href="/order/get/?mk=" + document.querySelector("#memKey").value;
    }
                ...
                <div id="contents">
                    ...
                    <div className="xans-element- xans-myshop xans-myshop-orderhistorytab ec-base-tab ">
                        <ul className="menu order_history_menu_ul">
                            <li className="tab_class order_history_width">
                                <Link to="/order/get">
                                    주문내역조회
                                </Link>
                            </li>
                            <li className="tab_class_cs selected">
                                <a href="#">취소/반품/교환 내역 (<span id="xans_myshop_total_orders_cs">{orderCount}</span>)</a>
                            </li>
                        </ul>
                    </div>
                        ...
                        <table border="1" summary="">
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" className="service">재주문</th>
                                </tr>
                            </thead>
                            <tbody>
                                {

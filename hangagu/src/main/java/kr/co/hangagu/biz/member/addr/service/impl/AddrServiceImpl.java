...
public class AddrServiceImpl implements AddrService {
	...
	public ResultDto findByAddrKey(AddrDto dto) { 
	 ...
	 /* 
	  ...
	  @Transactional public ResultDto save(CartDto dto) { ResultDto resultDto = new
	  ResultDto();
	  ...
	  for(int i = 0; i < dto.getCartDtoList().size(); i++) { CartEntity crt = new
	  CartEntity();
	  ...
	  if(newCart.size() == 0) {
	  resultDto.setData(HangaguConstant.ResponseEnum.NOT_EXIST_RESPONSE.getCode().
	  toString());
	  resultDto.setMessage(HangaguConstant.ResponseEnum.NOT_EXIST_RESPONSE.getDesc(
	  ).toString()); } else {
	  ...
	  if(!ipResultDto.getData().equals(HangaguConstant.ResponseEnum.SUCCESS.getDesc
	  ().toString())) {
	  resultDto.setData(HangaguConstant.ResponseEnum.NOT_EXIST_RESPONSE.getCode().
	  toString());
	  resultDto.setMessage(HangaguConstant.ResponseEnum.NOT_EXIST_RESPONSE.getDesc(
	  ).toString()); } else { resultDto.setData(newCart);
	  resultDto.setMessage(HangaguConstant.ResponseEnum.SUCCESS.getDesc().toString(
	  )); } } else { resultDto.setData(newCart);
	  resultDto.setMessage(HangaguConstant.ResponseEnum.SUCCESS.getDesc().toString(
	  )); }
